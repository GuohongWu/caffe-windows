#include "caffe/layers/proposal_rcnn_test.hpp"
...
namespace caffe
{
		...
		const int feat_stride)
	{
		...
		for (int h = 0; h < bottom_H; ++h)
		{
			for (int w = 0; w < bottom_W; ++w)
			{
				...
				{
					// pbox is stored as N x k x 4 x h x w,then have the following formula:
					...
					// pbox is stored as N x 4 x k x h x w, the have the following formula:
	...
	template <typename Dtype>
	void ProposalRCNNTestLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
		const vector<Blob<Dtype> *> &top)
	{

		ProposalRCNNTestParameter param = this->layer_param_.proposal_rcnn_test_param();
	...
	template <typename Dtype>
	void ProposalRCNNTestLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
		const vector<Blob<Dtype> *> &top)
	{
		// Note that we must process N images at one count
		// the probability map N x 2 x h x w
		const Dtype *p_bottom_item = bottom[0]->cpu_data();
		// the transformation map N x 4 x h x w
		const Dtype *p_d_anchor_item = bottom[1]->cpu_data();
		// the image info storest the w,h,
		...
		for (int n = 0; n < bottom[0]->shape(0); ++n)
		{
			...
			// input image height & width and scale factor
			...
			sort_box(proposals_.mutable_cpu_data(), 0, num_proposals - 1, pre_nms_topn);
			// nms for cpu data, the proposals may contain multiple information about the bounding box, such as mask,key point,class, ground truth transformation
...
#ifdef CPU_ONLY
	STUB_GPU(ProposalRCNNTestLayer);
#endif
	INSTANTIATE_CLASS(ProposalRCNNTestLayer);
	REGISTER_LAYER_CLASS(ProposalRCNNTest);
}