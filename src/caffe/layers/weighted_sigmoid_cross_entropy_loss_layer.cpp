...
namespace caffe {
	...
	void WeightedSigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
		...
		for (int i = 0; i < bottom[0]->count(); ++i) {
			...
			loss -= ((target_value == 1) ? pos_weight_ : neg_weight_) * (input_data[i] * (target[i] - (input_data[i] >= 0)) -
	...
	void WeightedSigmoidCrossEntropyLossLayer<Dtype>::Backward_cpu(
		...
		if (propagate_down[0]) {
			...
			caffe_mul(count, sigmoid_output_data, target, bottom_diff);  // Pn * Pn^
			caffe_cpu_axpby(count, this->neg_weight_, sigmoid_output_data, this->pos_weight_ - this->neg_weight_, bottom_diff);  // neg_weight_ * Pn^ + (pos_weight_ - neg_weight_) * bottom_diff
			caffe_axpy(count, -this->pos_weight_, target, bottom_diff);  // bottom_diff - pos_weight_ * Pn
