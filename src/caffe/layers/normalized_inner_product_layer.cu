...
namespace caffe {
	...
	template <typename Dtype>
	void NormalizedInnerProductLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	template <typename Dtype>
	void NormalizedInnerProductLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		...
		if (top.size() == 2 && this->param_propagate_down_[0]) {
			caffe_gpu_add(this->blobs_[0]->count(), top[1]->gpu_diff(), this->blobs_[0]->gpu_diff(), this->blobs_[0]->mutable_gpu_diff());
		}
	...
	INSTANTIATE_LAYER_GPU_FUNCS(NormalizedInnerProductLayer);
