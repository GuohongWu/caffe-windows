...
void ScalFixNormalizeLayer<Dtype>::LayerSetUp(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  normalize_type_ =
    this->layer_param_.scalfix_normalize_param().normalize_type();
  fix_gradient_ =
    this->layer_param_.scalfix_normalize_param().fix_gradient();
  bp_norm_ = this->layer_param_.scalfix_normalize_param().bp_norm() && top.size() == 2;
}

template <typename Dtype>
void ScalFixNormalizeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ScalFixNormalizeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ScalFixNormalizeLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(ScalFixNormalizeLayer);
#endif

INSTANTIATE_CLASS(ScalFixNormalizeLayer);
REGISTER_LAYER_CLASS(ScalFixNormalize);
